    ...
    protected FileSystem doCreateFileSystem(FileName fileName,
        ...
        if (service == null) {
            ...
            try {
                ...
                if (StringUtils.isBlank(key) && StringUtils.isEmpty(keyId) && StringUtils.isEmpty(domain)) {
                ...
                if (StringUtils.isNotBlank(domain)) {
                	try {
						service = new AmazonS3Client(AWSCredentialsProvider.class.cast(Class.forName(domain).newInstance()));
					} catch (Exception e) {
						throw new FileSystemException("Invalid Credentials Class", e);
					}
                } else {
